...
export default function Features() {
  ...
  return (
    <section ref={ref} className="section-container bg-perla">
      ...
      >
        <h2 className="text-caribe mb-4">{t.sectionTitle}</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-8">
        {t.items.map((feature: { title: string; description: string }, index: number) => {
          const highlight = FEATURE_HIGHLIGHTS[index] || false;
          return (
            <motion.div
              key={feature.title}
              initial={{ opacity: 0, y: 40 }}
